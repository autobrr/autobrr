...
func (c *Client) get(ctx context.Context, endpoint string) (int, []byte, error) {
	...
	if err != nil {
		return 0, nil, errors.Wrap(err, "whisparr.http.Do(req): %+v", req)
	}
	...
	if _, err = io.Copy(&buf, resp.Body); err != nil {
		return resp.StatusCode, nil, errors.Wrap(err, "whisparr.io.Copy")
	}
...
func (c *Client) getJSON(ctx context.Context, endpoint string, params url.Values, data any) error {
	...
	resp, err := c.http.Do(req)
	if err != nil {
		return errors.Wrap(err, "whisparr.http.Do(req): %+v", req)
	}
	...
	if resp.StatusCode == http.StatusUnauthorized {
		...
		return errors.New("whisparr: bad request: %s (status: %s)", endpoint, resp.Status)
	}
...
func (c *Client) postBody(ctx context.Context, endpoint string, data any) (int, []byte, error) {
	...
	if resp.StatusCode == http.StatusBadRequest {
		...
		return resp.StatusCode, buf.Bytes(), errors.New("whisparr: bad request: %v (status: %s): %s", resp.Request.RequestURI, resp.Status, buf.String())
	}
