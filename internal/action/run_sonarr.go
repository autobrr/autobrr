...
func (s *Service) runSonarr(ctx context.Context, action *domain.Action, release *domain.Release) ([]string, error) {
	l := zerolog.Ctx(ctx)
	...
	if err != nil {
		return nil, errors.Wrap(err, "sonarr: failed to push release: %s", req.Title)
	}
	...
	l.Debug().Str("indexer", req.Indexer).Str("host", cfg.Host).Msg("release successfully added to client")
