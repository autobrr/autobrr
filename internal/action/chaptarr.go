...
func (s *Service) chaptarr(ctx context.Context, action *domain.Action, release *domain.Release) ([]string, error) {
	l := zerolog.Ctx(ctx)

	l.Trace().Msg("running Chaptarr action")
	...
	arrClient, ok := client.Client.(*chaptarr.Client)
	if !ok {
	...
	if err != nil {
		return nil, errors.Wrap(err, "chaptarr: failed to push release: %v", r)
	}
