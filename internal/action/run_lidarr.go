...
func (s *Service) runLidarr(ctx context.Context, action *domain.Action, release *domain.Release) ([]string, error) {
	l := zerolog.Ctx(ctx)
	...
	if err != nil {
		return nil, errors.Wrap(err, "lidarr failed to push release: %s", req.Title)
	}
