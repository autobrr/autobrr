...
func NewPushoverSender(log zerolog.Logger, settings *domain.Notification) *pushoverSender {
	return &pushoverSender{
		baseSender: newBaseSender("pushover", settings),
...
func (s *pushoverSender) Send(ctx context.Context, payload domain.NotificationPayload) error {
...
func (s *pushoverSender) sound(event domain.NotificationEvent) string {
