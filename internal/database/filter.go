...
func (r *FilterRepo) FindByID(ctx context.Context, filterID int) (*domain.Filter, error) {
	...
	var f domain.Filter
	var minSize, maxSize, maxDownloadsUnit, matchReleases, exceptReleases, matchReleaseGroups, exceptReleaseGroups, matchReleaseTags, exceptReleaseTags, matchRawAnnounce, exceptRawAnnounce, freeleechPercent, shows, seasons, episodes, years, artists, albums, matchCategories, exceptCategories, matchUploaders, exceptUploaders, tags, exceptTags, extScriptCmd, extScriptArgs, extWebhookHost, extWebhookData sql.NullString
	...
	f.MatchRawAnnounce = matchRawAnnounce.String
	f.ExceptRawAnnounce = exceptRawAnnounce.String
...
func (r *FilterRepo) findByIndexerIdentifier(ctx context.Context, tx *Tx, indexer string) ([]domain.Filter, error) {
	queryBuilder := r.db.squirrel.
		Select(
			...
			"f.match_raw_announce",
			"f.except_raw_announce",
			"f.use_regex_raw_announce",
			"f.scene",
			"f.freeleech",
			"f.freeleech_percent",
			"f.smart_episode",
...
func (r *FilterRepo) Store(ctx context.Context, filter domain.Filter) (*domain.Filter, error) {
	queryBuilder := r.db.squirrel.
		...
		Values(
			...
			filter.MatchRawAnnounce,
			filter.ExceptRawAnnounce,
			filter.UseRegexRawAnnounce,
			filter.Scene,
			filter.Freeleech,
...
func (r *FilterRepo) Update(ctx context.Context, filter domain.Filter) (*domain.Filter, error) {
	...
	queryBuilder := r.db.squirrel.
		...
		Set("match_raw_announce", filter.MatchRawAnnounce).
		Set("except_raw_announce", filter.ExceptRawAnnounce).
		Set("use_regex_raw_announce", filter.UseRegexRawAnnounce).
...
func (r *FilterRepo) UpdatePartial(ctx context.Context, filter domain.FilterUpdate) error {
	...
	if filter.MatchRawAnnounce != nil {
		q = q.Set("match_raw_announce", filter.MatchRawAnnounce)
	}
	if filter.ExceptRawAnnounce != nil {
		q = q.Set("except_raw_announce", filter.ExceptRawAnnounce)
	}
	if filter.UseRegexRawAnnounce != nil {
		q = q.Set("use_regex_raw_announce", filter.UseRegexRawAnnounce)
	}
	if filter.Scene != nil {
