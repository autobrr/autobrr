...
func SQLiteMigrations(db *sql.DB, logger zerolog.Logger) *migrator.Migrator {
	...
	migrate.AddFileMigration("91_release_add_title_normalized.sql")
	migrate.AddMigration(&migrator.Migration{Name: "92_release_upsert_title_normalized", Run: func(db *sql.DB) error {
		...
		for {
			...
			rows, err := tx.Query(`SELECT id, title FROM "release" WHERE title_normalized IS NULL LIMIT ?`, batchSize)
			if err != nil {
				log.Error().Err(err).Msg("error querying rows")
				return err
			}

			ids := make([]int64, 0, batchSize)
			norms := make([]string, 0, batchSize)
			...
			for rows.Next() {
				...
				rows.Scan(&id, &title)
				ids = append(ids, id)
				norms = append(norms, domain.MustNormalize(title))
			}
			rows.Close()
			...
			logger.Debug().Int("batch", batchNum).Dur("duration", time.Since(start)).Str("migration", "92_release_upsert_title_normalized").Msg("completed database migration")

			stmt, _ := tx.Prepare(`UPDATE "release" SET title_normalized = ? WHERE id = ?`)
			for i := range ids {
				if _, err := stmt.Exec(norms[i], ids[i]); err != nil {
					stmt.Close()
					tx.Rollback()
					return err
				}
		...
		logger.Debug().Dur("duration", time.Since(start)).Str("migration", "92_release_upsert_title_normalized").Msg("completed database migration")
